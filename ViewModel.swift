...
class ViewModel:ObservableObject {
    ...
    func showTotal() -> some View {
        ...
        return Text("Total: $\(grandTotal, specifier: "%.2f")")
            .font(.custom("Poppins-SemiBoldItalic", size:28))
            .foregroundColor(/*@START_MENU_TOKEN@*/.blue/*@END_MENU_TOKEN@*/)
    }
    
    func showCostEach() -> some View {
        let show = data.numberOfPeople > 0 ? Text("$\(totalPerPerson, specifier: "%.2f") each").font(.custom("Poppins-SemiBoldItalic", size:28)).foregroundColor(/*@START_MENU_TOKEN@*/.blue/*@END_MENU_TOKEN@*/) : Text("")
        
        return show
    }
    
    func showSavings() -> some View {
        let show = totalPerPerson > 0.00 && data.numberOfPeople > 0 ? Text("Each person saves $\(totalSaved, specifier: "%.2f")").foregroundColor(.green).font(.custom("Poppins-SemiBoldItalic", size:28)) : Text("")
